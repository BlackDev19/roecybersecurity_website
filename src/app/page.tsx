...
import { FaLinkedin, FaTwitter, FaFacebook, FaYoutube, FaTiktok } from "react-icons/fa"
...
// Messages rotatifs pour le hero
const ROTATING_SLOGANS = [
  "🔒 Votre cybersécurité, notre mission. Votre avenir numérique, notre engagement.",
  "🚀 Innovez, apprenez et protégez vos données avec confiance.",
  "💻 La puissance de la technologie au service de la sécurité et de la liberté numérique.",
]
...
const CYBERSECURITY_SERVICES: Array<{
...
}> = [
  {
    ...
    description: "80 % des entreprises ayant subi une violation de données n'avaient not réalisé d'audit complet. Nos audits identifient les failles critiques avant qu'elles ne soient exploitées.",
    gradient: "from-slate-400 to-slate-500"
  },
  {
    icon: Eye,
    title: "Test d'Intrusion",
    description: "En 2022, plus de 493 millions d'attaques par ransomware ont été détectées dans le monde. Nos pentests simulent ces menaces pour renforcer vos défenses.",
    gradient: "from-indigo-400 to-indigo-500"
  },
  {
    icon: BookOpenCheck,
    title: "Formation",
    description: "20 % des violations sont dues à des erreurs humaines. Nos formations interactives réduisent ce risque en renforçant les compétences de vos équipes.",
    gradient: "from-emerald-400 to-emerald-500"
  },
  {
    icon: Award,
    title: "Certifications",
    description: "La certification ISO 27001 est reconnue mondialement et exigée dans les secteurs sensibles. Nous vous accompagnement vers ISO 27001, SOC 2 et RGPD.",
...
const useCarouselState = (itemsLength: number, autoPlayInterval = ANIMATION_CONFIG.intervals.carousel) => {
...
const ServiceCard = memo(({ icon: Icon, title, description, gradient }: ServiceCardProps) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl font-bold text-gray-900 mb-3 font-sans tracking-tight relative z-10 group-hover:text-blue-800 transition-colors duration-300">{title}</h3>
      ...
      <button className="mt-4 text-blue-600 font-semibold text-sm flex items-center gap-1 self-start hover:gap-2 transition-all duration-300 group-hover:text-blue-800 relative z-10">
        En savoir plus
...
const SpecificationCard = memo(({ icon: Icon, label, description, gradient }: SpecificationCardProps) => {
  const animations = createAnimationVariants(useReducedMotion() ?? false)

  return (
    <motion.div
      whileHover={{ y: -4, scale: 1.03, transition: { duration: ANIMATION_CONFIG.duration.fast, ease: ANIMATION_CONFIG.ease } }}
    ...
    >
      ...
      <div className="relative z-10">
        <h4 className="font-semibold text-white group-hover:text-blue-300 transition-colors duration-300">{label}</h4>
        <p className="text-gray-300 text-sm group-hover:text-blue-200 transition-colors duration-300">{description}</p>
      </div>
    </motion.div>
  )
})
...
interface CarouselControlsProps {
  currentIndex: number;
  totalItems: number;
  onPrevious: () => void;  // Correction ici
  onNext: () => void;      // Correction ici
  ...
  <>
    ...
    <button 
      ...
      className="absolute left-2 top-1/2 -translate-y-1/2 text-white 
                 bg-black/30 p-2 rounded-full hover:bg-black/50 
    ...
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
      {Array.from({ length: totalItems }, (_, index) => (
        <button
          key={index}
          className={`
            w-3 h-3 rounded-full cursor-pointer transition-colors duration-200 relative overflow-hidden group
...
const HeroSection = memo(() => {
  ...
  const slogans = Array.isArray(rawSlogans) ? rawSlogans.filter(s => typeof s === "string") as string[] : [];
  const currentSloganIndex = useRotatingContent(slogans);

  return (
    <section className="relative min-h-[85vh] flex flex-col items-center justify-start 
      ...
      >
        <div className="absolute -inset-4 bg-blue-500/10 rounded-full blur-2xl group-hover:bg-blue-500/20 transition-all duration-500" 
             aria-hidden="true" />
        <div className="h-44 w-44 sm:h-52 sm:w-52 rounded-3xl overflow-hidden 
        ...
        <div className="absolute -top-2 -right-2 bg-gradient-to-r from-emerald-500 
      ...
      >
        {t("hero.welcome")} <span className="bg-gradient-to-r from-sky-400 to-blue-600 bg-clip-text text-transparent">{t("hero.companyName")}</span>
      </motion.h1>
      ...
      <motion.div className="flex flex-wrap justify-center gap-5 mb-4">
        {NAVIGATION_BUTTONS.map((button, index) => (
          <div key={button.textKey} className="relative group">
            <NavigationButton
              icon={button.icon}
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <div className="relative w-full lg:w-1/3 rounded-3xl overflow-hidden 
            ...
            {/* Effet de lueur sur le carousel */}
            <div className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500">
              <div className="w-full h-full bg-gradient-to-br from-blue-500/10 to-purple-500/10 blur-xl" />
            </div>
          </div>
          ...
          <motion.div className="flex-1 flex flex-col items-center lg:items-start 
            ...
            <h2 className="text-3xl md:text-4xl font-bold mb-4 font-sans tracking-tight">{t("ordinateur.title")}</h2>
            <p className="text-lg text-gray-300 mb-4 font-medium">{t("ordinateur.subtitle")}</p>
  ...
  return (
    <section ref={sectionRef as Ref<HTMLElement>} className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4 font-sans tracking-tight">{t("services.title")}</h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto font-medium">{t("services.subtitle")}</p>
        </motion.div>
        ...
        >
          {((Array.isArray(tObject("services.list")) ? tObject("services.list") : []) as Array<{title?: string, description?: string}>).map((service, index) => (
            <motion.div
...
const CallToActionSection = memo(() => {
  ...
  const socialLinks = [
    { icon: <FaLinkedin size={28} />, link: "https://www.linkedin.com", bg: "bg-[#0A66C2]", name: "LinkedIn" },
    { icon: <FaTwitter size={28} />, link: "https://twitter.com", bg: "bg-[#1DA1F2]", name: "Twitter" },
    { icon: <FaFacebook size={28} />, link: "https://www.facebook.com/share/p/1BTZqnrS5U/?mibextid=wwXIfr", bg: "bg-[#1877F2]", name: "Facebook" },
    { icon: <FaYoutube size={28} />, link: "https://youtube.com", bg: "bg-[#FF0000]", name: "YouTube" },
    { icon: <FaTiktok size={28} />, link: "https://www.tiktok.com/t/ZP8BvuSBD/", bg: "bg-[#000000]", name: "TikTok" },
  ]

  const statsData = [{
    number: "60%", 
    label: "PME ciblées par des cyberattaques", 
    icon: ShieldCheck
  },
  {
    number: "300,000+",
  label: "Professionnels formés chaque année dans le monde",
  icon: BookOpenCheck
  },
  {
    number: "93%", 
    label: "Réseaux vulnérables détectés en entreprise", 
    icon: Award
  },
  {
    number: "24/7", 
    label: "Surveillance & réponse aux incidents", 
    icon: AlertCircle
  }]
    ...
    <section className="py-16 bg-gradient-to-br from-slate-900 via-slate-800 to-gray-900 text-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          >
            {((Array.isArray(tObject("callToAction.stats")) ? tObject("callToAction.stats") : []) as Array<{number?: string, label?: string}>).map((stat, index) => (
              <motion.div
                key={index}
              ...
              >
                ...
                {index === 0 && <ShieldCheck className="h-8 w-8 mx-auto mb-3 text-blue-400 relative z-10 group-hover:scale-110 transition-transform duration-300" />}
                {index === 1 && <BookOpenCheck className="h-8 w-8 mx-auto mb-3 text-violet-400 relative z-10 group-hover:scale-110 transition-transform duration-300" />}
                {index === 2 && <Award className="h-8 w-8 mx-auto mb-3 text-amber-400 relative z-10 group-hover:scale-110 transition-transform duration-300" />}
                {index === 3 && <AlertCircle className="h-8 w-8 mx-auto mb-3 text-rose-400 relative z-10 group-hover:scale-110 transition-transform duration-300" />}
...
>
  ...
  <div className="relative group">
    ...
    <div className="absolute -inset-2 bg-gradient-to-r from-blue-500/30 to-purple-500/30 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl pointer-events-none" />
          ...
          >
            <h3 className="text-xl md:text-2xl font-semibold text-slate-200 mb-6 font-sans tracking-tight">{t("callToAction.socialsTitle")}</h3>
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <div className="text-center mb-8">
            <h3 className="text-2xl md:text-3xl font-bold text-white mb-3 font-sans tracking-tight">{t("footer.title")}</h3>
            <p className="text-gray-400 max-w-2xl mx-auto font-medium">{t("footer.description")}</p>
          </div>

          {/* Badges de certification */}
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            {Object.entries(tObject("footer.certifications") as Record<string, string>).map(([key, value]) => (
              <motion.div
                key={key}
          ...
          <div className="border-t border-gray-700 pt-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-3 text-gray-400 group relative overflow-hidden">
                ...
                <span className="font-medium relative z-10 group-hover:text-blue-300 transition-colors duration-300">{t("footer.copyright", { default: `© ${new Date().getFullYear()} ROECybersecurity - Tous droits réservés` }).replace("{year}", new Date().getFullYear().toString())}</span>
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className="text-gray-500 font-medium group-hover:text-blue-300 transition-colors duration-300">{t("footer.developed")}</span>
              </div>
            </div>
          </div>
        </motion.div>
