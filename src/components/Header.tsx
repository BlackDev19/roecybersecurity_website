...
function getLinkStyles(isActive: boolean, variant: 'desktop' | 'mobile' = 'desktop') {
  const base =
    'group relative flex items-center gap-3 rounded-2xl transition-all duration-300 overflow-hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/60 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-900'
  const desktop = 'px-4 py-2.5 text-sm font-medium'
  const mobile = 'px-5 py-4 text-base font-medium'
  const activeStyles =
    'text-blue-100 bg-gradient-to-r from-blue-600/20 to-blue-500/10 border border-blue-400/20 shadow-lg shadow-blue-500/5'
...
const MobileNavigation = React.memo<{ isOpen: boolean; pathname: string; onClose: () => void }>(
  ({ isOpen, pathname, onClose }) => {
    ...
    useEffect(() => {
      ...
      const focusableEls = Array.from(
        document.querySelectorAll('#mobile-nav a, #mobile-nav button')
      ) as HTMLElement[]
                    ...
                    return (
                      ...
                      >
                        <Link
                          href={href}
                          onClick={onClose}
                          className={getLinkStyles(isActive, 'mobile')}
                        >
                          {Icon && (
                            <Icon className="h-5 w-5 transition-transform duration-300 group-hover:scale-110" />
                          )}
